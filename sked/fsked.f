...
C   INPUT
      character*(*) cfile ! file name specified on command line
C
C   Functions
...
!      nch = len(cfile)
! AEM 20050305 use proper 'trimlen' instead of 'len' to avoid attempts to open './.skd' at start
      nch = trimlen(cfile)
      cmdline=cfile
      cmdline_old="wh"
