...
C
C  CABL1 determines the cable wrap setting for the first source.
C  There is no previous source, so we don't have a "come from"
C  cable wrap. If there is a unique wrap, use that, otherwise
C  pick one.
C
